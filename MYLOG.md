...
现在也想考虑敌反馈机制的处理，因为敌反馈机制不应该直接写在主角的类里面，这样会造成代码臃肿
并且有时候“主角Actor”并没有操作的时候，也可能会存在别的“敌人Actor”主动碰撞到主角
这时候就无法接受到伤害反馈之类的内容，所以需要敌人也能有一些触发机制的处理
现在有可能还是要增加一个 Computer 类自动注册进每一个actor 里面
可以使用 Actor.computer 的方式获取到这个类，然后像是覆盖 controller 的函数一样
在外部生成行为循环，或者伤害触发。覆盖 computer 内部的函数即可。

假如存在一个敌人向你开了一枪，那么伤害触发究竟写在什么地方好呢？是的，最好生成一个移动的子弹 Actor
然后让触发伤害的内容写在子弹里面是最好的。
