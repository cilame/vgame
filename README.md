...
# 1 某个方向跳跃的最大高度，不限制高度会无限起飞。
#  *注意，这个高度最好适当调整一些 y 方向上的加速度、重力、最大速度调整到最佳游戏性
# 2 某个方向跳跃的最大次数，跳跃后直到落到地面才会重新更新次数
#  *注意，如果不设置这个参数，默认跳跃次数无限，类似游戏 flappy bird 的模式。
#   一般游戏跳跃的次数设置 1 或 2 已经足够

# “重力系统” 专用参数
a.physics.limit_highs = {8:80} #1
a.physics.jump_times = {8:2} #2
...
- ##### 开始用这个简单的系统写一个游戏，下面这个代码是一个简单的带有 二段跳，枪械使用以及切换的游戏演示
